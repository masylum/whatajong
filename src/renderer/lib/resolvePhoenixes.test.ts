...
describe("resolvePhoenixRun", () => {
  it("initializes phoenix run when phoenix card is played with no active run", () => {
    ...
    expect(game.phoenixRun!.card).toBe("pb")
  ...
  it("sets the number to the tile rank when a suit card is played", () => {
    const game: Game = {
      points: 0,
      phoenixRun: { card: "pb", number: 2, combo: 0 },
    }
  ...
  it("increments the number when a rabbit is played", () => {
    const game: Game = {
      points: 0,
      phoenixRun: { card: "pb", number: 3, combo: 3 },
    }
  ...
  it("wraps around the run when the number reaches 8", () => {
    const game: Game = {
      points: 0,
      phoenixRun: { card: "pb", number: 8, combo: 8 },
    }
  ...
  it("maintains the run for special cards", () => {
    const game: Game = {
      points: 0,
      phoenixRun: { card: "pb", number: 4, combo: 6 },
    }
    ...
    game.phoenixRun = { card: "pb", number: 4, combo: 6 }
